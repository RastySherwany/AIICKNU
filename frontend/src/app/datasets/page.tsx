...
export default function Page() {
  ...
  useEffect(() => {
    fetch(`http://${window.location.hostname}:3001/dataset`)
      .then(res => res.json())
      .then(resData => {
        setData(resData);
        setLoading(false);
  ...
  return (
    <div className="pt-32 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[70vh]">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
        <h1 className="text-4xl font-extrabold tracking-tight text-[#002147] mb-4">Datasets</h1>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {data.map((item, idx) => (
            ...
            >
              <Link href={`/datasets/${item.id}`} className="block h-full group">
