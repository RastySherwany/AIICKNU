...
export default function DetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`http://${window.location.hostname}:3001/activity/` + params.id)
      .then(res => res.json())
      .then(resData => {
        setData(resData);
        setLoading(false);
  ...
  return (
    <div className="pt-32 pb-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[70vh]">
      <Link href="/activities" className="inline-flex items-center text-[#002147] hover:text-blue-900 mb-8 transition-colors">
